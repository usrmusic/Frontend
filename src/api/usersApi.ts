...
// Roster for the DJ Colours settings page. Backend membership is role 3
// (Staff/DJ) OR anyone who has ever been assigned as an event's dj_id —
// matters because at least one Admin-role user is the busiest DJ in the
// system and would otherwise be excluded by a role-only filter.
export const useDjColors = () => {
  return useQuery<{ data: DjColor[] }>({
    queryKey: ["dj-colors"],
    queryFn: async () => {
      const response = await AxiosInstance.get("/user/dj-colors");
      return response.data;
    },
  });
};

export const useSaveDjColor = () => {
  const queryClient = useQueryClient();
  return useMutation({
    // Reuses PUT /user/:id (the existing partial-update route) — color is one
    // of the fields it accepts. Empty string clears back to the grey fallback.
    mutationFn: async ({ id, color }: { id: number; color: string | null }) => {
      const response = await AxiosInstance.put(`/user/${id}`, { color: color ?? "" });
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["dj-colors"] });
      // Both calendars read the DJ colour off events — refresh them too.
    ...
    onError: (error: unknown) => {
      if (axios.isAxiosError(error)) {
        const msg = error.response?.data;
        toast.error(msg?.error || "Failed to save colour");
      }
    },
  });
};
